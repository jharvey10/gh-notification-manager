...
import { formatNotificationType } from '../../shared/formatNotificationType.mjs';
  ...
  return (
    <tr>
      ...
      <td className="w-full">
        ...
        {url &&
          <button
            className={clsx('btn btn-link text-left font-normal', isUnread ? 'text-primary' : 'text-gray-400')}
            type="button"
            onClick={handleOpen}
          >
            {title}
      ...
      <td className="w-40"><small>{repo}</small></td>
      <td className="w-full">
        {tags.map((tag) => (
          <TagBadge key={tag} tag={tag} active />
        ))}
      </td>
      <td className="w-40"><small>{time}</small></td>
    </tr>
  );
}
